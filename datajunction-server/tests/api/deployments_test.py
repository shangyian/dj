...
def _mock_materializing_query_service() -> MagicMock:
    """
    A query service mock whose materialization calls return serializable info.
    """
    mock_qs = MagicMock()
    info = MaterializationInfo(urls=["http://fake.url/job"], output_tables=[])
    mock_qs.materialize_cube.return_value = info
    mock_qs.get_materialization_info.return_value = info
    mock_qs.deactivate_cube_workflow.return_value = {"status": "deactivated"}
    mock_qs.deactivate_workflows.return_value = {"status": "deactivated"}
    return mock_qs
