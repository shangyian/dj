...
class Coalesce(Function):
    """
    Returns the first non-null expression in the input list.
    """
    @staticmethod
    def infer_type(*args) -> ct.ColumnType:
        # Since the output type of COALESCE is the same as the type of the first non-null expression,
        # we can just return the type of the first argument.
        return args[0]
